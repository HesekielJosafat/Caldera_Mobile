...
import 'user_ticket_payment_screen.dart'; 

class UserTicketDetailScreen extends StatefulWidget {
  final Map<String, dynamic> ticket;

  const UserTicketDetailScreen({Key? key, required this.ticket}) : super(key: key);
...
class _UserTicketDetailScreenState extends State<UserTicketDetailScreen> {
  ...
  Widget _buildStatusBadge(String text, Color color) {
    return Container(
      ...
      child: Text(text, style: GoogleFonts.sora(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(value ?? '-', style: GoogleFonts.sora(fontSize: 12, color: Colors.black87, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Ambil data tiket dari widget parameter
      ...
      body: SingleChildScrollView(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Text("Kode Tiket: ${ticket['ticket_code']}", style: GoogleFonts.sora(fontSize: 14, color: Colors.grey.shade600, fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  _buildStatusBadge(status == 'active' ? "Aktif" : (status == 'used' ? "Digunakan" : "Tidak Aktif"), status == 'active' ? Colors.green : Colors.grey),
                  const SizedBox(width: 8),
                  _buildStatusBadge(paymentStatus == 'paid' || paymentStatus == 'payment_verified' ? "Lunas" : "Belum Bayar", paymentStatus == 'unpaid' ? Colors.orange : Colors.green),
                ],
              ),
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("Informasi Customer", style: GoogleFonts.sora(fontWeight: FontWeight.bold, fontSize: 12, color: primaryNavy)),
                        const SizedBox(height: 12),
                        _buildInfoRow("Nama", ticket['customer_name']),
                        _buildInfoRow("Email", ticket['customer_email']),
                        _buildInfoRow("Telepon", ticket['customer_phone']),
                      ],
                    ),
                  ),
                  const SizedBox(width: 16),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("Informasi Tiket", style: GoogleFonts.sora(fontWeight: FontWeight.bold, fontSize: 12, color: primaryNavy)),
                        const SizedBox(height: 12),
                        _buildInfoRow("Tanggal", _formatDate(ticket['visit_date'])),
                        _buildInfoRow("Jenis Tiket", ticketType),
                        _buildInfoRow("Jumlah", "${ticket['number_of_tickets']} tiket"),
                        _buildInfoRow("Total", formatCurrency.format(double.tryParse(ticket['total_amount']?.toString() ?? '0'))),
                      ],
                    ),
                  ),
                ],
              ...
              if (paymentStatus == 'unpaid') ...[
                ...
                Container(
                  padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(8)),
                  child: Row(
                    children: [
                      const Icon(Icons.warning_amber_rounded, color: Colors.orange, size: 16), const SizedBox(width: 8),
                      Expanded(child: Text("Silakan lakukan pembayaran untuk mengaktifkan tiket Anda.", style: GoogleFonts.sora(fontSize: 10, color: Colors.orange.shade900))),
                    ],
                  ),
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () async {
                      final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => UserTicketPaymentScreen(ticketCode: ticket['ticket_code'], amount: double.parse(ticket['total_amount'].toString()))));
                      if (result == true) Navigator.pop(context, true);
                    },
                    icon: const Icon(Icons.upload, color: Colors.white, size: 16),
                    label: Text("Upload Bukti Pembayaran", style: GoogleFonts.sora(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                  ),
                ),
              ],
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  style: OutlinedButton.styleFrom(side: BorderSide(color: Colors.grey.shade400), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                  onPressed: () => Navigator.pop(context),
                  child: Text("Kembali", style: GoogleFonts.sora(color: Colors.grey.shade700, fontWeight: FontWeight.bold, fontSize: 12)),
                ),
              )
            ],
          ),
