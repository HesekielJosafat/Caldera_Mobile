...
class UserTicketPaymentScreen extends StatefulWidget {
  final String ticketCode;
  final double amount;

  const UserTicketPaymentScreen({Key? key, required this.ticketCode, required this.amount}) : super(key: key);

  @override
  State<UserTicketPaymentScreen> createState() => _UserTicketPaymentScreenState();
}

class _UserTicketPaymentScreenState extends State<UserTicketPaymentScreen> {
  ...
  Future<void> _uploadPayment() async {
    if (_selectedBank == null || _accountNameCtrl.text.isEmpty || _selectedImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lengkapi Bank, Nama, dan Bukti Transfer!")));
      return;
    }
    ...
    final result = await _apiService.uploadTicketPayment(widget.ticketCode, payload, _selectedImage!);
              ...
              child: Column(
                ...
                children: [
                  Center(
                    child: Column(
                      children: [
                        const Icon(Icons.upload_file, size: 40, color: Color(0xFFD4AF37)),
                        const SizedBox(height: 12),
                        Text("Upload Bukti Pembayaran", style: GoogleFonts.playfairDisplay(fontSize: 20, fontWeight: FontWeight.bold, color: const Color(0xFF14334C))),
                        Text("Kode Tiket: ${widget.ticketCode}", style: GoogleFonts.sora(fontSize: 12, color: Colors.grey.shade600)),
                      ],
                    ),
                  ),
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        isExpanded: true, hint: Text("Pilih Bank", style: GoogleFonts.sora(fontSize: 13, color: Colors.grey)), value: _selectedBank,
                        items: ['BCA', 'MANDIRI', 'BRI', 'BNI', 'Lainnya'].map((c) => DropdownMenuItem(value: c, child: Text(c, style: GoogleFonts.sora(fontSize: 13)))).toList(),
                  ...
                  TextField(controller: _accountNameCtrl, decoration: InputDecoration(hintText: "Contoh: JOHN DOE", border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 14))),
                  const SizedBox(height: 16),
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: _selectedImage != null
                          ...
                          : Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.add_photo_alternate, size: 32, color: Colors.grey.shade400), Text("Pilih foto (Max 2MB)", style: GoogleFonts.sora(fontSize: 10))]),
                    ),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF14334C), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), 
                      onPressed: _uploadPayment, 
                      child: Text("Upload Sekarang", style: GoogleFonts.sora(color: Colors.white, fontWeight: FontWeight.bold))
                    )
                  )
                ],
              ),
